...
export default function ReversiPage() {
	...
	const [message, setMessage] = useState<MessageType>({ message: null, useSpinner: false });

	useEffect(() => setupGameControlInterval(gameControl, setCompleteMessage, setIsModalOpen, setMessage, setStatus), [gameControl]);
	useEffect(() => initializeGameControl(searchParams, gameControl), [searchParams, gameControl]);
	...
	return (
		<div className="flex w-screen h-screen flex-row bg-yellow-50">
			...
			<MainContent
				...
				message={message}
				handleCellClick={(x: number, y: number) => updateGameControlWithMove(gameControl, x, y, setStatus)}
...
const initializeGameControl = (searchParams: URLSearchParams, gameControl: GameController) => {
...
const MainContent = ({ gameControl, message, handleCellClick, handleGiveUp }: MainContentProps) => (
	<div className="w-1/2 h-full flex flex-col gap-3 p-3 items-center">
		...
		<Board gameBoard={gameControl.boardController.getCurrentBoard()} onClick={handleCellClick} />
...
const ConfirmModalWrapper = ({ isModalOpen, completeMessage, handleModalOK, setIsModalOpen }: ConfirmModalWrapperProps) => (
	<ConfirmModal
		isOpen={isModalOpen}
		title={"ゲーム終了"}
