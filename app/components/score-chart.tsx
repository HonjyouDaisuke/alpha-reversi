...
export default function ScoreChart({ score }: Props) {
	return (
		<div className=" flex flex-row rounded h-12 w-full p-2 gap-2 shadow bg-green-200">
			<div className="w-16 text-right text-black">黒:{score.blackCount}</div>
			<div className={"flex h-full flex-row w-full"}>
				{getPostArray(score).map((c, index) => {
					return <PutCell key={index} p={c} />;
				})}
			</div>
			<div className="w-16 text-black">{score.whiteCount}:白</div>
		</div>
	);
}
