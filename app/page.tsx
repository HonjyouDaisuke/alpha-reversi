...
export default function Home() {
  ...
  const [turn, setTurn] = useState<Turn>(new Turn(0, 1));
  ...
  const [dummyState, setDummyState] = useState(false); // ダミーの状態変数
  ...
  function handleCellClick(x: number, y: number) {
    ...
    setTurn(gameControl.turnControl.getCurrentTurn());
  ...
  useEffect(() => {
    console.log(`ターンが変わりました`);
    gameControl.turnControl.printTurn();
  }, [turn]);
  return (
